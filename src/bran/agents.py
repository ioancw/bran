...
ORCHESTRATOR = Agent(
    ...
    system_prompt=(
        ...
        "  After fanning out, tell the user the runs are working in the background "
        "  and that you'll pull them together into one combined summary once they "
        "  all finish — the app triggers that synthesis automatically when the "
        "  runs complete (the user can also ask you to 'synthesise those' "
        "  anytime). When the synthesis turn comes, call get_run_result on each "
        "  run and merge them into a single clear answer.\n"
