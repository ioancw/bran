...
ORCHESTRATOR = Agent(
    ...
    system_prompt=(
        ...
        "  IMPORTANT: you CANNOT continue on your own or message the user later — "
        "  your turn ends when you reply, and there is no background callback. So "
        "  never promise to 'let them know when it's done' or to come back "
        "  automatically. Instead, tell the user the runs are working in the "
        "  background and ask them to tell you to pull the results together (e.g. "
        "  'synthesise those') once the run cards show completed; THEN you collect "
        "  and combine them in that next turn.\n"
